...
## No need to save HTMLS - add to `.html` to the `.gitignore` file to make sure htmls are not saved. Stage, commit, comment, push, and pull

### FORMATTING TABS

- use a level 3 header `###` and `{.tabset .tabset-fade}` followed by a level 4 header and puts everything following into the tabs. 
- new tab is created using another level 4 tab.
- here we added in a new tab after pika but before the level 3 header
...
# Info on this cutie!
## Add a picture with a file in your folders `![]()`

![Ring-tail Cat - Courtesy of Virigina Zoo](img/ringtail_cat.jpg)

## we'll also learn about `knitrr::include_graphics()` as another way to put pictures into your document

## they can rotate their hindfeet 180 degrees. Not endangered species but very elusive. Closely related to racoons
